...
///
/// \see RegIterator
#define RegEntry(reg, iterator) ((TK_CONCAT(__TK_RegType_, reg) *)((uint8_t *)&(iterator)->next - offsetof(TK_CONCAT(__TK_RegType_, reg), __TK_regElem.next)))

//
//
//
// Checks whether `elem` is the head.
static inline bool __TK_RegIsHead(RegElem* elem) { return elem != NULL && elem->prev == NULL && elem->next != NULL; }

// Checks whether `elem` is an interior element.
static inline bool __TK_RegIsInterior(RegElem* elem) { return elem != NULL && elem->prev != NULL && elem->next != NULL; }

// Checks whether `elem` is the tail.
static inline bool __TK_RegIsTail(RegElem* elem) { return elem != NULL && elem->prev != NULL && elem->next == NULL; }
...
size_t __TK_RegSize(Reg* reg) {
	size_t res = 0;
	for (RegIterator it = RegBegin(*reg); it != RegEnd(*reg); it = RegNext(it)) ++res;
	return res;
}
