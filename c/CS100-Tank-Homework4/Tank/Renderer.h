...
void RdrPutChar(Vec pos, char c, Color color) {
	...
	if (color == TK_AUTO_COLOR) {
		Flag flag = (Flag)c;
		if (flag == eFlagNone) color = TK_NORMAL;
		else if (flag == eFlagSolid) color = TK_BLUE;
		else if (flag == eFlagWall) color = TK_WHITE;
		else if (flag == eFlagTank) color = TK_INVALID_COLOR;
		else color = TK_INVALID_COLOR;
	}
...
void RdrClear(void) {
	...
	for (RegIterator it = RegBegin(regTank); it != RegEnd(regTank); it = RegNext(it)) {
		...
		Vec pos = tank->pos;
		for (int y = -1; y <= 1; ++y) for (int x = -1; x <= 1; ++x) RdrPutChar(Add(pos, (Vec){x, y}), ' ', TK_AUTO_COLOR);
	}
	// Clear bullets.
...
void RdrRender(void) {
	...
	for (RegIterator it = RegBegin(regTank); it != RegEnd(regTank); it = RegNext(it)) {
		...
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir), dir == eDirL || dir == eDirR ? '-' : '|', color);
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionAntiClockwise[dir]), '@', color);
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionAntiClockwise[dir]), '@', color);
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionAntiClockwise[dir]), '@', color);
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionAntiClockwise[dir]), '*', color);
		dir = tank->dir;
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionClockwise[dir]), '@', color);
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionClockwise[dir]), '@', color);
		RdrPutChar(LVec$_add_LVec$LDir$_LVec$(pos, dir = directionClockwise[dir]), '@', color);
