...
int Rand(int n) {
	TK_ASSERT(n <= RAND_MAX, "`n` is too large to be handled by `Rand`, please implement your own random number generator");
	return rand() % n;
}

/// \brief Randomly generate a `double` in `[0, 1)`.
double Rand01(void) { return (double)rand() / ((double)RAND_MAX + 1.0); }
...
bool Eq(Vec a, Vec b) { return a.x == b.x && a.y == b.y; }

bool Neq(Vec a, Vec b) { return !Eq(a, b); }

Vec Add(Vec a, Vec b) { return (Vec){a.x + b.x, a.y + b.y}; }

Vec Sub(Vec a, Vec b) { return (Vec){a.x - b.x, a.y - b.y}; }

Vec Mul(Vec a, int b) { return (Vec){a.x * b, a.y * b}; }

/// \brief Randomly generate a `Vec` in `[0, v.x) * [0, v.y)`.
Vec RandVec(Vec v) {
	TK_ASSERT(v.x <= RAND_MAX && v.y <= RAND_MAX, "`v` is too large to be handled by `RandVec`, please implement your own random number generator");
